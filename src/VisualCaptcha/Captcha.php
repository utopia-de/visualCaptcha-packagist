...
class Captcha {
	...
	/**
	 *
	 * @return \Zend\Cache\Storage\StorageInterface|boolean
	 */
	...
	public function streamAudio( Response $response, $fileType ) {
		...
		if ( ! empty( $audioFileName ) ) {
			...
			return $this->utilStreamFile( $response, $audioFilePath );
		}
	...
	public function streamImage( Response $response, $index, $isRetina ) {
		...
		if ( ! empty( $imageFileName ) ) {
			return $this->utilStreamFile( $response, $imageFilePath );
		}
	...
	public function validateAudio( $sentOption ) {
		...
		return ( in_array( strtoupper( $sentOption ), $validAnswers, true ) );
	}
	...
	public function getImageOptionAtIndex( $index ) {
		...
		return $imageOptions[ $index ] ?? null;
	}
	...
	private function utilStreamFile( Response $response, $filePath ) {
		if ( ! file_exists( $filePath ) ) {
			return $response;
		}
		...
		$img .= $this->utilRandomHex( random_int( 0, 1500 ) );
	...
	private function getMimeType( $filePath ) {
		...
		if ( function_exists( 'finfo_open' ) ) {// Use finfo (right way)
			...
			if ( $pathinfo = pathinfo( $filePath ) ) {
				...
				if ( getimagesize( $filePath ) && in_array( $pathinfo['extension'], $imagetypes, true ) ) {
